/**
 * Audio Engine - Core audio processing functionality
...
class AudioEngine {
	...
	async startRecording() {
		...
		try {
			...
			// Initialize MediaRecorder for WAV recording
			...
			this.mediaRecorder.onstop = async () => {
				...
				await this.loadAudioFromBlob(blob, "Recording " + Date.now());
				this.recordingData = [];
			};
	...
	playTrack(trackId, startTime = 0, duration = null) {
		...
		source.onended = () => {
			...
			if (this.playingSources.size === 0) {
				this.isPlaying = false;
				this.onPlaybackFinished && this.onPlaybackFinished();
			}
		};
	}
	...
	stop() {
		...
		for (const [trackId, source] of this.playingSources) {
			try {
				source.stop();
			} catch (error) {
	...
	getTotalDuration() {
		let maxDuration = 0;
		for (const [id, data] of this.audioBuffers) {
