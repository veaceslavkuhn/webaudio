...
describe('AudioEngine Advanced Features', () => {
    ...
    describe('Waveform Generation', () => {
        test('should generate sine wave correctly', () => {
            const trackId = audioEngine.generateWaveform('sine', 440, 1, 1);
            
            expect(trackId).toBeTruthy();
            expect(mockAudioContext.createBuffer).toHaveBeenCalledWith(1, 44100, 44100);
        });

        test('should generate triangle wave correctly', () => {
            const trackId = audioEngine.generateWaveform('triangle', 220, 1, 2);
            
            expect(trackId).toBeTruthy();
            expect(mockAudioContext.createBuffer).toHaveBeenCalledWith(1, 44100, 44100);
        });

        test('should generate sawtooth wave correctly', () => {
            const trackId = audioEngine.generateWaveform('sawtooth', 110, 1, 1);
            
            expect(trackId).toBeTruthy();
            expect(mockAudioContext.createBuffer).toHaveBeenCalledWith(1, 44100, 44100);
        });

        test('should generate square wave correctly', () => {
            const trackId = audioEngine.generateWaveform('square', 880, 1, 0.5);
            
            expect(trackId).toBeTruthy();
            expect(mockAudioContext.createBuffer).toHaveBeenCalledWith(1, 44100, 44100);
        });

        test('should handle unknown waveform type', () => {
            const trackId = audioEngine.generateWaveform('unknown', 440, 1, 1);
            
            expect(trackId).toBeTruthy(); // Should default to sine
    ...
    describe('Volume and Playback Rate', () => {
        test('should set master volume correctly', () => {
            ...
            expect(audioEngine.masterGain.gain.setValueAtTime).toHaveBeenCalledWith(0.5, 0);
        });
    ...
    describe('Utility Methods', () => {
        ...
        test('should get all track IDs', () => {
            audioEngine.audioBuffers.set('track1', { name: 'Track 1' });
            audioEngine.audioBuffers.set('track2', { name: 'Track 2' });
            
            const trackIds = audioEngine.getAllTrackIds();
            
            expect(trackIds).toEqual(['track1', 'track2']);
        });

        test('should check if track exists', () => {
            audioEngine.audioBuffers.set('existing-track', { name: 'Existing' });
            
            expect(audioEngine.hasTrack('existing-track')).toBe(true);
            expect(audioEngine.hasTrack('non-existing')).toBe(false);
        });
    });
